...
#include "perceptron-2classes-wh.h"
...
int perceptron_2c_wh_load_random_pattern(Pattern_2c_wh *pattern) {
    if (rand() % 2 == 0) {
        return perceptron_2c_wh_load_pattern("zero.txt", pattern);
    } else {
        return perceptron_2c_wh_load_pattern("un.txt", pattern);
    }
}
...
int perceptron_2c_wh_neuron_propagation(const Perceptron_2c_wh *perceptron, const Pattern_2c_wh *pattern) {
    const float potential_i = perceptron_2c_wh_calculate_potential(perceptron, pattern);
    return perceptron_2c_wh_heaviside_activation_function(perceptron, potential_i);
}

void perceptron_2c_wh_learning(Perceptron_2c_wh *perceptron, const Pattern_2c_wh *pattern, const float error) {
    if (error != 0) {
        for (int i = 0; i < NB_PIXELS; i++) {
            perceptron->weights[i] += perceptron->epsilon * error * (float)pattern->pixels[i];
        }
        perceptron->theta += perceptron->epsilon * error * 1.0f;
    }
}
...
void perceptron_2c_wh_train_perceptron(Perceptron_2c_wh *perceptron, int max_iterations) {
    ...
    do {
        ...
        perceptron_2c_wh_load_random_pattern(&pattern);
        const float potential_i = perceptron_2c_wh_calculate_potential(perceptron, &pattern);
        perceptron->errors[pattern.class] = (float)pattern.class - potential_i;
        perceptron_2c_wh_learning(perceptron, &pattern, perceptron->errors[pattern.class]);

        for (int i = 0; i < NB_CLASSES; i++) {
            total_error += fabsf(perceptron->errors[i]);
        }
    ...
    perceptron_2c_wh_draw_training_plot();
    perceptron_2c_wh_load_pattern("zero.txt", &pattern);
    printf("Distance à l'hyperplan 0 : %f", perceptron_2c_wh_distance_a_hyperplan(perceptron, &pattern));
    perceptron_2c_wh_load_pattern("un.txt", &pattern);
    printf("Distance à l'hyperplan 1 : %f\n", perceptron_2c_wh_distance_a_hyperplan(perceptron, &pattern));
}
...
void perceptron_2c_wh_test_perceptron(const Perceptron_2c_wh *perceptron) {
    ...
    printf("\n--- Test avec motif '0' ---\n");
    perceptron_2c_wh_load_pattern("zero.txt", &pattern);
    perceptron_2c_wh_display_pattern(&pattern);
    int result = perceptron_2c_wh_neuron_propagation(perceptron, &pattern);
    printf("Result: %d (expected: %d)\n", result, pattern.class);

    printf("\n--- Test avec motif '1' ---\n");
    perceptron_2c_wh_load_pattern("un.txt", &pattern);
    perceptron_2c_wh_display_pattern(&pattern);
    result = perceptron_2c_wh_neuron_propagation(perceptron, &pattern);
    printf("Result: %d (expected: %d)\n", result, pattern.class);
}
...
float perceptron_2c_wh_test_pattern_generalisation(const Perceptron_2c_wh *perceptron, char *filename, float noise_percentage, int nb_tests) {
    Pattern_2c_wh original_pattern, noisy_pattern;
    int errors = 0;

    perceptron_2c_wh_load_pattern(filename, &original_pattern);

    for (int i = 0; i < nb_tests; i++) {
        perceptron_2c_wh_noise_pattern(&original_pattern, &noisy_pattern, noise_percentage);

        int output = perceptron_2c_wh_neuron_propagation(perceptron, &noisy_pattern);
...
void perceptron_2c_wh_create_generalisation_graph(const Perceptron_2c_wh *perceptron, const int nb_points, const int nb_tests_per_point) {
    GeneralisationResult_2c_wh results[nb_points];
    ...
    for (int i = 0; i < nb_points; i++) {
        ...
        const float zero_error_rate = perceptron_2c_wh_test_pattern_generalisation(perceptron, "zero.txt", noise_percentage, nb_tests_per_point);
        const float one_error_rate = perceptron_2c_wh_test_pattern_generalisation(perceptron, "un.txt", noise_percentage, nb_tests_per_point);
...
void perceptron_2c_wh_run(int choice_weight_init){
    ...
    Perceptron_2c_wh perceptron;
    perceptron_2c_wh_init_perceptron(&perceptron, choice_weight_init);
    perceptron_2c_wh_display_weights(&perceptron);

    printf("Start learning\n");
    perceptron_2c_wh_train_perceptron(&perceptron, 10000);

    printf("Test");
    perceptron_2c_wh_test_perceptron(&perceptron);

    perceptron_2c_wh_create_generalisation_graph(&perceptron, 101, 100);
    perceptron_2c_wh_display_weights(&perceptron);
}
